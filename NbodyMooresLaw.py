...
for year,data in papers.items():
    ...
    if year==1952:
        lbl = "Simulation Efficiency [Myr/CPU month]"
    else:
        lbl=None
    if inner:
...
# Cosmetics
plt.yscale('log')
ax.set_ylim(10**(-4.5),10**(5.5))
ax.set_yticks(10.**np.arange(-4,6))
minor_tx = np.array([np.arange(2,10) * pow10 for pow10 in 10.**np.arange(-4,6)])
